...
const useQueue = !process.env.VERCEL && Boolean(process.env.REDIS_HOST || process.env.REDIS_URL);

// BullMQ requires this option on the ioredis connection.
export const redisConnection = useQueue
...
export const imageAnalysisQueue = useQueue
  ? new Queue<ImageAnalysisJobData>(IMAGE_ANALYSIS_QUEUE, {
